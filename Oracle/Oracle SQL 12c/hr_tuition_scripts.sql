...
(SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE JOB_ID LIKE '%SA%'
);
ROLLBACK;
COMMIT;
--savepoints
DELETE FROM EMPLOYEES_COPY WHERE JOB_ID = 'IT_PROG';
SAVEPOINT A;
DELETE FROM EMPLOYEES_COPY WHERE JOB_ID = 'SA_MAN';
SAVEPOINT B;
DELETE FROM EMPLOYEES_COPY WHERE JOB_ID = 'AD_VP';
SAVEPOINT C;
DELETE FROM EMPLOYEES_COPY WHERE JOB_ID = 'PU_CLERK';
SAVEPOINT D;
ROLLBACK TO B;
ROLLBACK;
ROLLBACK A;
...
SELECT *
FROM EMPLOYEES
WHERE JOB_ID = 'IT_PROG' FOR UPDATE NOWAIT;
SELECT FIRST_NAME,
LAST_NAME,
SALARY
FROM EMPLOYEES E
...
FROM EMPLOYEES E
JOIN DEPARTMENTS D USING (DEPARTMENT_ID)
WHERE LOCATION_ID = 1400 FOR UPDATE OF FIRST_NAME,
LAST_NAME;
--DDL
...
SELECT * FROM EMPLOYEES WHERE 1 = 2; --never equal so no data copied,only table created
CREATE TABLE IT_PROG AS
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG';
...
(FIRST_NAME, LASTNAME, SALARY
) AS
SELECT FIRST_NAME, LAST_NAME, SALARY FROM EMPLOYEES;
CREATE TABLE EMP_COPY2
(FIRST_NAME, LASTNAME, SALARY
) AS
SELECT FIRST_NAME, LAST_NAME, SALARY FROM EMPLOYEES WHERE 1 = 2;
CREATE TABLE EMP_COPY3
(FIRST_NAME, LAST_NAME, ANNUAL_SALARY
) AS
SELECT FIRST_NAME, LAST_NAME, SALARY*12 FROM EMPLOYEES;
--alter table
ALTER TABLE EMP_COPY1 ADD EMAIL      VARCHAR(50);
ALTER TABLE EMP_COPY1 ADD BIRTH_DATE DATE DEFAULT SYSDATE;
ALTER TABLE EMP_COPY ADD (EMAIL VARCHAR(50), BIRTH_DATE DATE DEFAULT SYSDATE);
ALTER TABLE EMP_COPY1 MODIFY EMAIL VARCHAR(100);
ALTER TABLE EMP_COPY1 MODIFY EMAIL VARCHAR(100) DEFAULT 'default@email.com';
ALTER TABLE EMP_COPY MODIFY (EMAIL VARCHAR(50), BIRTH_DATE DATE DEFAULT (SYSDATE-10));
ALTER TABLE EMP_COPY1
DROP COLUMN EMAIL;
ALTER TABLE EMP_COPY
DROP (EMAIL , BIRTH_DATE);
--drop table
...
--comment
COMMENT ON COLUMN IT_PROG.JOB_ID
IS
'Abbreviation of Job Titles';
COMMENT ON COLUMN IT_PROG.SALARY
IS
'IT programmers salaries';
COMMENT ON COLUMN IT_PROG.EMAIL
IS
'';
COMMENT ON TABLE IT_PROG
IS
'Table for IT Programmers.';
COMMENT ON TABLE IT_PROG
IS
'';
SELECT * FROM USER_TAB_COMMENTS WHERE table_name = 'IT_PROG';
SELECT * FROM USER_COL_COMMENTS WHERE table_name = 'IT_PROG';
--rename tables
DESC EMP_COPY1;
ALTER TABLE EMP_COPY1 RENAME COLUMN BIRTH_DATE TO DOB;
RENAME EMP_COPY1 TO EMPLOYEES_COPY1;
ALTER TABLE EMPLOYEES_COPY1 RENAME TO EMP_COPY1;
--Constraints
--NOT NULL
CREATE TABLE MANAGERS
(
FIRST_NAME VARCHAR(50) NOT NULL,
LAST_NAME  VARCHAR(50) CONSTRAINT LNAME_CONS NOT NULL,
...
CREATE TABLE MANAGERS
(
EMPLOYEE_ID NUMBER CONSTRAINT PK_EMPLOYEEID PRIMARY KEY,
FIRST_NAME VARCHAR(50) NOT NULL UNIQUE,
LAST_NAME  VARCHAR(50) CONSTRAINT LNAME_CONS UNIQUE NOT NULL,
SALARY     NUMBER(6) DEFAULT 0,
DEPARTMENT_ID NUMBER NOT NULL,
PHONE_NUMBER VARCHAR(10) UNIQUE NOT NULL,
CONSTRAINT DEPARTMENT_CONS UNIQUE(DEPARTMENT_ID),
CONSTRAINT COMPOSED_UNIQUE UNIQUE(EMPLOYEE_ID, FIRST_NAME,LAST_NAME)
);
...
ALTER TABLE MANAGERS ADD CONSTRAINT PK_EMPLOYEE_ID PRIMARY KEY (EMPLOYEE_ID);
...
LAST_NAME  VARCHAR(50) CONSTRAINT LNAME_CONS UNIQUE NOT NULL,
SALARY     NUMBER(6) ,
CONSTRAINT MIN_SALARY CHECK (SALARY > 100)
);

insert into managers values (1, 'fname','lname', 199);